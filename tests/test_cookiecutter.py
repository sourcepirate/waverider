...
class TestCookiecutterTemplate:
    ...
    def test_cookiecutter_json_structure(self, template_dir):
        ...
        assert (
            "{{" in config["project_slug"]
        ), "project_slug should use cookiecutter template syntax"
        assert (
            "cookiecutter.project_name" in config["project_slug"]
        ), "project_slug should derive from project_name"
    ...
    def test_requirements_files_exist(self, template_dir):
        ...
        requirements_dir = os.path.join(
            template_dir, "{{ cookiecutter.project_slug }}/requirements"
        )
        ...
        for oauth2_file in oauth2_files:
            ...
            with open(file_path, "r") as f:
                ...
                if oauth2_file == "providers.py":
                    ...
                    ), "providers.py should contain Google provider"
                    assert (
                        "github" in content.lower()
                    ), "providers.py should contain GitHub provider"
                    assert (
                        "facebook" in content.lower()
    ...
    def test_tests_package_structure(self, template_dir):
        ...
        assert os.path.exists(
            os.path.join(tests_dir, "test_api_package.py")
        ), "Main test file should exist"
    ...
    def test_manage_py_exists(self, template_dir):
        ...
        with open(manage_path, "r") as f:
            ...
            assert (
                "django.core.management" in content
            ), "manage.py should import Django management"
            assert (
                "execute_from_command_line" in content
            ), "manage.py should use execute_from_command_line"
            assert (
                "{{ cookiecutter.project_slug }}" in content
            ), "manage.py should reference project slug"
...
class TestCookiecutterGeneration:
    ...
    def test_generated_project_structure(self, temp_dir, template_dir, test_context):
        ...
        generated_project = cookiecutter(
            template_dir, no_input=True, extra_context=test_context, output_dir=temp_dir
        )
