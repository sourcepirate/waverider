...
def normalize_user_data(
    provider: str, user_info: Dict[str, Any], access_token: Optional[str] = None
) -> Dict[str, Any]:
    """
...
def _normalize_github_data(
    user_info: Dict[str, Any], access_token: str = None
) -> Dict[str, Any]:
...
def _normalize_facebook_data(user_info: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "username": (
            user_info.get("email", "").split("@")[0] if user_info.get("email") else ""
        ),
        "provider_id": user_info.get("id"),
    }
